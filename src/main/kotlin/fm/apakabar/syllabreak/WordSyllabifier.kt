...
class WordSyllabifier(private val rule: LanguageRule) {
    ...
    fun syllabifyWord(word: String, softHyphen: String): String {
...
private class WordSyllabification(
    ...
    private fun extractConsonantCluster(left: Int, right: Int): Pair<List<SyllableToken>, List<Int>> {
    ...
    private fun findClusterBetweenNuclei(nk: Int, nk1: Int): Pair<List<SyllableToken>, List<Int>> {
        val left = skipSeparatorsForward(nk + 1)
        val right = skipSeparatorsBackward(nk1 - 1)
        return extractConsonantCluster(left, right)
    }
    
    private fun isValidOnset(consonant1: String, consonant2: String, prevNucleusIdx: Int? = null): Boolean {
        ...
        return when (cluster.size) {
            0 -> {
                ...
                if (rule.glides.isEmpty() && rule.digraphVowels.isEmpty()) {
                    ...
                    val allSeparators = (nk + 1 until nk1).all { 
